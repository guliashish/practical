...
def register():
    ...
    mycursor.execute("select uid from mca4a")
    table_uid=mycursor.fetchall()
    for item in table_uid:
        if item == uid:
            print("ashish")
            #Label(frame,text="Uid already exists in database",width=50,height=5,fg='red').grid(row=9,column=0)
            
        else:pass
"""
            sql=("insert into mca4a values(%s,%s,%s,%s,%s,%s)")
...
btnaa=Button(frame,text="register", command=register).grid(row=8,column=1)
